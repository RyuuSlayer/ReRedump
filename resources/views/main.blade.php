<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Recent Dumps Section -->
            <div class="bg-white dark:bg-discord-200 overflow-hidden shadow-xl sm:rounded-lg mb-6">
                <div class="p-6">
                    <h2 class="text-xl font-semibold mb-4 dark:text-white">Recent dumps</h2>
                    <div class="space-y-2">
                        <!-- This will be populated dynamically -->
                        <div class="recent-dumps">
                            <!-- Example structure (will be replaced by dynamic data) -->
                            <div class="text-sm text-gray-600 dark:text-gray-300">
                                <!-- Dynamic content will go here -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- News Section -->
            <div class="bg-white dark:bg-discord-200 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <h2 class="text-xl font-semibold mb-4 dark:text-white">News</h2>
                    <div class="space-y-4">
                        <!-- This will be populated dynamically -->
                        <div class="news-items">
                            <!-- Example structure (will be replaced by dynamic data) -->
                            <div class="news-item">
                                <!-- Dynamic content will go here -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
